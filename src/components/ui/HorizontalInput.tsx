...
export default function HorizontalInput({ label, colSpan, hasToBeProportional, defaultValue, fixedValue, disabled, fieldId, readOnly, isError, errorMessage, setErrors, onValueChange }: { label: string, colSpan?: string, hasToBeProportional: boolean, defaultValue?: string, fixedValue?: string, disabled?: boolean, fieldId: string, readOnly?: boolean, isError: boolean, errorMessage: string | null, setErrors: () => void, onValueChange?: (val: string) => void }) : React.ReactElement {
    ...
    useEffect(() => {
        if(disabled && fixedValue === undefined){
            setValue('');
        }
    }, [disabled, fixedValue]);
